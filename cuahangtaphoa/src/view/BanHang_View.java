...
//				sp_DAO.deleteMuaHang();
		...
		btnNewButton_4 = new JButton("Quay lại");
		...
		btnMua.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (jtfSoLuong.getText().equals("0")) {
					JOptionPane.showMessageDialog(null, "Sản phẩm đã hết hàng không thể thêm");
				} else {
		...
		JButton btnXoaMH = new JButton("Làm mới");
...
//				 sp_DAO.deleteMuaHang();
				...
				try {
					...
					XSSFSheet sheet = workbook.createSheet("Danh sách mua hàng");
					XSSFRow row = null;
					Cell cell = null;
